...
export_model<-rbind(summary.gam(physical_model_s)$p.table, summary.gam(psych_model_s)$p.table, summary.gam(social_model)$p.table, summary.gam(envir_model_s)$p.table)

write.csv(export_model, here::here("tables","20220722_Model_summaries.csv"))
```
...
pred_text<-data.frame(label= c("Adj R2=0.55", "Adj R2=0.43", "Adj R2=0.38", "Adj R2=0.45"), 
                       domain=c("Physical health", "Psychological health",  "Social relationships",  "Environmental health"), 
                       x = c(-40, -40, -40, -40), 
                       y = c(20, 20, 20, 20))

pred_text$domain<-factor(pred_text$domain, levels=c("Physical health", "Psychological health",  "Social relationships",  "Environmental health"))
